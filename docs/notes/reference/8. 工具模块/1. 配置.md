...
`apply_runtime_overrides(*, debug=MISSING, plugins_dir=MISSING, hot_reload=MISSING)` 用于 CLI / `BotClient` 传参：仅影响当前进程内的 `effective_*`，**不会**写入 `config.yaml`。`MISSING` 表示不覆盖该项。`clear_runtime_overrides()` 清空覆盖（测试或复用进程时可用）。
