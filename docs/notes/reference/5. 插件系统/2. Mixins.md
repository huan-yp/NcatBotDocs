...
### 1.3 wait_group_message_event() / wait_private_message_event()
...
等待下一条群聊或私聊消息，并直接返回对应的 QQ 事件实体。`predicate` 接收高层事件实体，因此可以直接读取 `group_id`、`user_id`、`raw_message` 等属性；返回值也可以直接调用 `reply()`、`delete()`、`ban()` 等实体方法。

| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `predicate` | `Callable[[GroupMessageEvent | PrivateMessageEvent], bool] \| None` | `None` | 实体过滤函数，`None` 接受该类型的任意消息 |
| `timeout` | `float \| None` | `None` | 超时秒数，`None` 无限等待 |
