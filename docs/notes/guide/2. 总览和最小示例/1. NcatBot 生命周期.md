...
主动模式需要你自行完成引导，使用如下代码即可：

```python
from ncatbot.core import BotClient
bot = BotClient() # 保留 bot 实例用于日后正常退出
api = bot.run_blocking(bt_uin="123456") # 这里写你自己的 BotQQ 号
```

完成引导后会返回一个 `BotAPI` 实例，这里就是 `api`，使用这个 `api` 可以调用 NcatBot 的接口，接下来就是你的时间了。

### 插件模式引导

插件模式一般把引导程序作为主程序，一个经典的引导程序如下：

```python
from ncatbot.core import BotClient
BotClient().run(bt_uin="123456")
```

执行 `BotClient().run()` 后控制权移交给插件系统，紧接着程序阻塞，直至 NcatBot 退出。
...
3. **确认登录**：通过 NapCat WebUI 接口确认登录结果，如果登录信息不一致，则终止运行并提醒。可以通过 `BotClient.run()` 中加入 `enable_webui_interaction=False` 跳过。
...
2. 处理事件
   - 事件总线外注册的回调函数处理事件。
   - 事件进入事件总线，激活**功能函数**, 调用订阅了该事件的所有**回调函数**。
3. 事件被拦截或者被处理
