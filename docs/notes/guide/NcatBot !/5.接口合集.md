...
permalink: /guide/interface_collection/
---
## ncatbot接口

---
这个是ncatbot的核心部分，ncatbot对接了napcat几乎全部接口，目前已知的是发送伪装合并转发的接口没有对接，其他的接口全部实现对接。
除此之外，接口的使用兼容了HTTP和WEBSOCKET服务端，也就是通俗的正向HTTP和WEBSOCKET。
系统默认使用的是WEBSOCKET，如果你想要使用HTTP使用接口，你需要在实例化`BotClient`时添加参数：
```python
