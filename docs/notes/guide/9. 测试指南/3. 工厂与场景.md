...
manifests = discover_testable_plugins(Path("docs/docs/examples/"))
for m in manifests:
...
在 `conftest.py` 中注册 pytest 插件，可使用 `--plugins-dir` 和 `@pytest.mark.plugin`：
...
```bash
python -m pytest --plugins-dir=docs/docs/examples/ -v
```
...
| 资源 | 链接 |
|------|------|
| Harness 详解 | [2.harness.md](<2. 测试工具.md>) |
| 测试 API 参考 | [reference/testing/](<../../reference/9. 测试指南/>) |
| Factory + Mock 完整签名 | [reference/testing/2_factory_scenario_mock.md](<../../reference/9. 测试指南/2. 工厂场景与 Mock.md>) |
