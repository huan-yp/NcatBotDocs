...
如果你了解 ssh, vim 两款工具以及基本的 Python 知识, 那么你可以直接使用云上镜像部署 NcatBot, 免去安装环境等繁琐步骤.
...
建立一个==新的工作目录==(文件夹), 比如建一个 `ncatbot` 文件夹, 在 `ncatbot` 中执行下面的代码运行 NcatBot (==需要更改代码中的 `bot_uin` 和 `root`==).
...
3. 复制执行下面的代码(==记得把代码中的 `bot_uin` 改成 Bot 的 QQ 号, 把 `root` 改成你的 QQ 号==)并回车执行. 
...
2. 复制以下代码(==记得把代码中的 `bot_uin` 改成 Bot 的 QQ 号, 把 `root` 改成你的 QQ 号==), 保存到工作目录下 `main.py` 中.

3. 在终端中输入 `python3 main.py` 执行代码, 执行过程中可能会提示**需求重启服务(紫色页面)**, 参考[FAQ](../7.%20常见问题/1.%20FAQ.md) 操作.
