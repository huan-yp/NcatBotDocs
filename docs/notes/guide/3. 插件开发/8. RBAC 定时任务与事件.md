...
| `wait_group_message_event(predicate=None, timeout=None) -> GroupMessageEvent` | 等待 QQ 群消息实体 |
| `wait_private_message_event(predicate=None, timeout=None) -> PrivateMessageEvent` | 等待 QQ 私聊消息实体 |

### 等待类型化消息实体

需要直接读取消息属性或调用 `reply()` 时，使用类型化等待方法可以省去对底层 `Event.data` 的转换：

```python
import asyncio

@registrar.on_group_command("下一条")
async def wait_next_message(self, event: GroupMessageEvent):
    await event.reply("请在 30 秒内发送下一条消息")
    try:
        reply = await self.wait_group_message_event(
            predicate=lambda e: (
                e.group_id == event.group_id and e.user_id == event.user_id
            ),
            timeout=30,
        )
...
私聊场景使用 `wait_private_message_event()`，其 predicate 和返回值均为 `PrivateMessageEvent`。两个方法都会忽略其他事件类型，并在超时时抛出 `asyncio.TimeoutError`。
