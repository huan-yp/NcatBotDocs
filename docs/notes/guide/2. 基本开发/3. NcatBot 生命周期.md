...
一般地，为了保障安全性，建议设置`ws_token`/`webui_token`令牌，长度应**至少12**位，且至少含有**数字、大小写字母及特殊符号**，如下：
...
1. 如果是公网监听模式（`ws_listen_ip = '0.0.0.0'`），检查 `ws_token` 强度，弱令牌会被强制修改。
2. 如果启用了 WebUI（`enable_webui = True`），检查 `webui_token` 强度，弱令牌会被强制修改。
